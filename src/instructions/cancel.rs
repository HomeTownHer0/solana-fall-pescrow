...
pub fn process_cancel_instruction(
    ...
    if derived != *escrow_account.address() {
    ...
    {
        let maker_ata = pinocchio_token::state::Account::from_account_view(maker_ata_a)?;
        if maker_ata.owner() != maker.address() {
            return Err(ProgramError::IllegalOwner);
        }
        if maker_ata.mint() != mint_a.address() {
            return Err(ProgramError::InvalidAccountData);
        }
    }
